...
export async function GET(request: Request) {
  ...
  try {
    const res = await fetch(
      `https://api.stlouisfed.org/fred/series/observations?series_id=${seriesId}&api_key=${process.env.FRED_API_KEY}&file_type=json&observation_start=${year}-01-01&observation_end=${year}-12-31`,
    ...
    const prevYearRes = await fetch(
      `https://api.stlouisfed.org/fred/series/observations?series_id=${seriesId}&api_key=${
        process.env.FRED_API_KEY
      }&file_type=json&observation_start=${
        parseInt(year) - 1
      }-12-01&observation_end=${parseInt(year) - 1}-12-31`
    );
    const prevYearData = await prevYearRes.json();
    const decPrevYearCPI = parseFloat(
      prevYearData.observations.slice(-1)[0].value
    );

    const monthlyRates = observations
      .map((obs, index: number) => {
        ...
        return {
          month: new Date(obs.date).toLocaleString("default", {
            month: "long",
          }),
          rate,
        };
    ...
    const yearlyRate = (
      ((decCPI - decPrevYearCPI) / decPrevYearCPI) *
      100
    ).toFixed(2);
